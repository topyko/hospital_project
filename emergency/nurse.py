...
class Nurse(StaffMember):
    ...
    def generate_report(patients):
        isolated = list(filter(lambda x: x.requires_isolation == True, patients))
        positive = list(filter(lambda x: x.diag == 'Positive', patients))
        children = list(filter(lambda x: x.personal_data["age"] <=16, patients))
        sick_children = list(filter(lambda x: x.personal_data["age"] <=16, positive))
        sick_senior = list(filter(lambda x: x.personal_data["age"] > 65, positive))
